...
namespace Nova.Common.Noise
{
    ...
    /// </summary>
    ///
    /// Edited by Levia so that it returns values in the range of 0..1
    public class SimplexNoise : INoise
    {
        ...
        {
            Seed = seed;
            float[,] vals = Calc2D(width, height, 0.1f);

            float[,] newVals = new float[width, height];

            for (int y = 0; y < height; y++)
            for (int x = 0; x < width; x++)
                newVals[x, y] = vals[x, y] / 255;
